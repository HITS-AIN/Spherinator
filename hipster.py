...
if __name__ == "__main__":
    myHipster = Hipster("/local_data/AIN/Data/HiPSter", "IllustrisV2", max_order=4, hierarchy=8, crop_size=256, output_size=64)
    myModel = models.RotationalSphericalAutoencoder()
    #checkpoint = torch.load("illustris.epoch824step82500.ckpt")
    checkpoint = torch.load("Illustris128_128_E99_S10000.ckpt")
    #checkpoint = torch.load("large.ckpt")
    #myModel = models.RotationalSphericalVariationalAutoencoder(h_dim=256, z_dim=3, distribution="vmf")
    #checkpoint = torch.load("vmf.ckpt")
